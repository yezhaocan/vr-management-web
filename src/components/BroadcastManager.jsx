...
export function BroadcastManager({
  ...
  return <div className="space-y-6">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-xl font-semibold text-white">播报管理</h3>
          <p className="text-gray-400">配置录像中的语音播报点</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <Card className="bg-gray-800/50 border-2 border-gray-600">
            <CardContent className="p-6">
              <h4 className="text-white font-medium text-lg mb-4">添加播报配置</h4>
              
              {/* 触发时间 */}
              <div className="space-y-3 mb-4">
                <Label className="text-white font-medium">触发时间（秒）*</Label>
                <Input type="number" value={newBroadcast.triggerTime || ''} onChange={e => handleInputChange('triggerTime', e.target.value)} placeholder="例如：10" className="bg-gray-700 border-gray-600 text-white p-3 rounded-lg w-full" min="0" />
              </div>

              {/* 文字稿 */}
              <div className="space-y-3 mb-4">
                <Label className="text-white font-medium">文字稿 *</Label>
                <Textarea value={newBroadcast.text || ''} onChange={e => handleInputChange('text', e.target.value)} placeholder="请输入播报文字内容" className="bg-gray-700 border-gray-600 text-white p-3 rounded-lg h-24 w-full" rows={4} />
              </div>

              {/* 字幕文件上传区域（新增） */}
              <div className="space-y-3 mb-4">
                <Label className="text-white font-medium">字幕文件（可选）</Label>
                ...
                {newBroadcast.subtitleFileId && <div className="relative">
                    <div className="flex items-center space-x-2 p-3 bg-blue-500/10 rounded-lg border border-blue-500/20">
                      ...
                      <div className="flex-1">
                        <span className="text-blue-400 text-sm font-medium">字幕文件已准备</span>
                        <p className="text-blue-400/70 text-xs mt-1">文件ID: {newBroadcast.subtitleFileId.substring(0, 20)}...</p>
                      </div>
              ...
              <div className="space-y-4 mb-4">
                <Label className="text-white font-medium">语音合成</Label>
              ...
              <div className="space-y-3">
                <Label className="text-white font-medium">上传音频文件 *</Label>
                ...
                <div className="relative">
                  ...
                  <label htmlFor="audio-file-upload" className={`flex flex-col items-center justify-center w-full h-20 border-2 border-dashed rounded-xl transition-all duration-200 cursor-pointer group ${uploadingAudio ? 'border-gray-400/50 bg-gray-900/10 cursor-not-allowed' : 'border-blue-400/50 bg-blue-900/10 hover:bg-blue-900/20'}`}>
                    <div className="flex flex-col items-center justify-center">
                      <Upload className={`w-6 h-6 mb-1 transition-colors ${uploadingAudio ? 'text-gray-400' : 'text-blue-400 group-hover:text-blue-300'}`} />
                ...
                {/* 音频文件状态显示 */}
                {newBroadcast.audioFileId && <div className="flex items-center space-x-2 p-3 bg-green-500/10 rounded-lg border border-green-500/20">
                    <CheckCircle className="h-4 w-4 text-green-400" />
                    <div>
                      <span className="text-green-400 text-sm font-medium">音频文件已准备</span>
                      <p className="text-green-400/70 text-xs mt-1">文件ID: {newBroadcast.audioFileId.substring(0, 20)}...</p>
                    </div>
                  </div>}
              </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-white font-medium text-lg">已配置播报</h4>
            <Badge className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full">
              {broadcasts.length}个播报点
            </Badge>
          </div>

          {broadcasts.length === 0 ? <Card className="bg-gray-800/30 border border-gray-600">
              <CardContent className="p-6 text-center">
                <Megaphone className="h-12 w-12 text-gray-500 mx-auto mb-3" />
                <p className="text-gray-400">暂无播报配置</p>
                <p className="text-gray-500 text-sm mt-1">在左侧添加第一个播报点</p>
              </CardContent>
            </Card> : <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {broadcasts.map((broadcast, index) => <Card key={broadcast.id} className="bg-gray-800/30 border border-gray-600 hover:border-blue-500/30 transition-colors">
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <Clock className="h-4 w-4 text-blue-400" />
                          <span className="text-white font-medium">第 {broadcast.triggerTime} 秒</span>
                          <Button type="button" className="h-6 px-2 bg-transparent border-0 hover:bg-blue-500/10">
                            <Play className="h-3 w-3" />
                          </Button>
                        </div>
                        <p className="text-gray-300 text-sm bg-gray-900/20 rounded p-3 mb-2">
                          {broadcast.text}
                        </p>
                        {broadcast.audioFileId && <div className="flex items-center space-x-2 text-green-400 text-xs mb-1">
                            <CheckCircle className="h-3 w-3" />
                            <span>音频文件已关联 (ID: {broadcast.audioFileId.substring(0, 12)}...)</span>
                          </div>}
                        {broadcast.subtitleFileId && <div className="flex items-center space-x-2 text-purple-400 text-xs">
                            <FileText className="h-3 w-3" />
                            <span>字幕文件已关联 (ID: {broadcast.subtitleFileId.substring(0, 12)}...)</span>
                          </div>}
                      </div>
                      <Button type="button" onClick={() => deleteBroadcast(index)} className="px-2 py-1 border border-red-400 text-red-400 hover:bg-red-400/10 bg-transparent ml-2">
