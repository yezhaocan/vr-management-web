...
// 背景音乐上传组件
function BackgroundMusicUploader({ cloudStorageId, onCloudStorageIdChange, $w }) {
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500 text-xs">
                  云存储ID: {cloudStorageId.substring(0, 12)}...
                </p>
              </div>
              <Button
                variant="ghost"
...
export function RouteEditor(props) {
  const { route, onClose, onSuccess, $w } = props;
  ...
  const checkAndSwitchMode = (lat, lng, waypoints = formData.waypoints) => {
    ...
    // 尝试通过坐标匹配已存在的航点
    const existingIndex = waypoints.findIndex(
      (wp) =>
        Math.abs(wp.lat - validatedLat) < 0.00000001 && Math.abs(wp.lng - validatedLng) < 0.00000001
    );
  ...
  const handleMapLocationSelect = (location) => {
    try {
      ...
      if (typeof location.index !== 'undefined') {
        ...
        toast({
          title: '编辑模式',
          description: `正在编辑: ${formData.waypoints[index].name}`,
          duration: 2000,
        });
        return;
      }
      ...
      <div className="lg:col-span-1 space-y-4 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar h-full">
          <WaypointForm
            ...
            onDeleteWaypoint={handleDeleteCurrentWaypoint}
            scenicCenter={scenicCenter}
        ...
        <Card className="bg-card border-border shadow-sm flex-1 flex flex-col min-h-0">
          ...
          <CardContent className="p-0 relative flex-1">
            <SimpleMap
              ...
              onClearConnections={clearConnectionsTrigger}
      ...
      <div className="flex flex-col h-full min-h-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto border border-border rounded-lg bg-background/50 p-2 custom-scrollbar">
          <WaypointList
            ...
            onSelectWaypoint={setSelectedVoiceIndex}
            selectedVoiceIndex={selectedVoiceIndex}
            locked={true} // 启用列表锁定功能
          />
        </div>
      </div>
      <div className="flex flex-col h-full min-h-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
          <VoiceConfigPanel
            waypoint={selectedVoiceIndex !== null ? formData.waypoints[selectedVoiceIndex] : null}
            onVoiceConfigChange={(field, value) =>
              updateWaypointVoiceConfig(selectedVoiceIndex, field, value)
            }
            onSynthesisComplete={handleSpeechSynthesisComplete}
            $w={$w}
  ...
  // 渲染背景音乐标签页
  const renderMusicConfigTab = () => (
    <div className="space-y-6">
      <div
        className="flex items-center space-x-2 cursor-pointer"
        onClick={() => handleInputChange('hasBackgroundMusic', !formData.hasBackgroundMusic)}
      >
        <input
          type="checkbox"
          checked={formData.hasBackgroundMusic}
          onChange={(e) => handleInputChange('hasBackgroundMusic', e.target.checked)}
      ...
      {formData.hasBackgroundMusic && (
        <BackgroundMusicUploader
          cloudStorageId={formData.cloudStorageId}
          onCloudStorageIdChange={handleCloudStorageIdChange}
          $w={$w}
        />
      )}
    </div>
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className="max-w-7xl min-h-[600px] max-h-[90vh] bg-background border border-border shadow-xl rounded-lg flex flex-col p-0 overflow-hidden"
