...
export function VideoList({ videos, onPlayVideo, onEditVideo, onDeleteVideo }) {
  const getStatusBadge = (status) => {
    ...
    return (
      <Badge variant={variants[status]} className="absolute top-2 right-2 z-10">
        {labels[status]}
      </Badge>
    );
  };
    ...
    <div className="grid grid-cols-1 gap-6">
      {videos.map((video) => (
        ...
        >
          ...
          <div className="text-xs text-muted-foreground mt-2">上传时间: {video.uploadTime}</div>
