      ...
      <Card className="bg-card border-border shadow-sm rounded-lg h-full overflow-hidden">
        ...
        <CardContent className="space-y-6 pt-6 overflow-y-auto">
          <div className="flex items-center justify-between bg-muted/30 p-3 rounded-lg border border-border">
            ...
            <Switch
              checked={voiceGuide.enabled}
              onCheckedChange={(checked) => onVoiceConfigChange('enabled', checked)}
            />
          </div>
          ...
          {voiceGuide.enabled && (
            <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder="请输入语音讲解文字（最多500字）"
                  value={voiceGuide.text}
                  onChange={(e) => onVoiceConfigChange('text', e.target.value)}
                  rows={4}
                  maxLength={500}
                  className="common-input resize-y min-h-[100px]"
                />
