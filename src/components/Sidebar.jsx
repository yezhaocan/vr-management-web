...
export function Sidebar(props) {
  const { activeMenu, onMenuChange, onLogout, $w } = props;
    ...
    <div className="w-64 bg-slate-900 border-r border-slate-700 flex flex-col h-full">
      ...
      <div className="p-4 border-b border-slate-700 bg-slate-800">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-white font-bold text-lg tracking-tight">无人机飞控中心</h1>
          </div>
        </div>
      </div>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Button
              key={item.id}
              variant="ghost"
              className={`w-full justify-start text-left transition-all duration-200 rounded-xl border border-transparent ${isActive ? 'bg-blue-600 text-white shadow-lg border-blue-400' : 'text-slate-300 hover:bg-slate-700 hover:text-white hover:border-slate-600'}`}
              onClick={() => onMenuChange(item.id)}
            >
              <div
                className={`w-1 h-8 rounded-full mr-3 transition-all duration-200 ${isActive ? 'bg-blue-300' : 'bg-transparent'}`}
              ></div>
              <Icon
                className={`w-5 h-5 mr-3 transition-colors duration-200 ${isActive ? 'text-white' : 'text-slate-400'}`}
              />
      ...
      <div className="p-4 border-t border-slate-700 bg-slate-800">
        <Button
          variant="ghost"
          className="w-full justify-start text-left text-slate-300 hover:bg-red-600 hover:text-red-100 border border-transparent rounded-xl transition-all duration-200"
          onClick={onLogout}
        >
