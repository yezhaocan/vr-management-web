...
export function WaypointList({
  ...
  const hasVoiceRecording = (waypoint) => {
    return (
      waypoint.voiceGuide &&
      waypoint.voiceGuide.enabled &&
      waypoint.voiceGuide.audioFileId &&
      waypoint.voiceGuide.audioFileId.trim() !== ''
    );
  };
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="space-y-2">
        {waypoints.map((waypoint, index) => (
          <div
            key={waypoint.id}
            className={`flex items-center justify-between px-4 rounded-lg border cursor-pointer transition-all h-[56px] group ${selectedVoiceIndex === index ? 'bg-primary/10 border-primary' : 'bg-card border-border hover:bg-muted/50'}`}
          ...
          >
            <div className="flex items-center space-x-3 overflow-hidden">
              ...
              {/* 语音讲解标识 - 适配深色模式 */}
              {hasVoiceRecording(waypoint) && (
                <div className="flex items-center text-xs text-primary bg-primary/10 px-1.5 py-0.5 rounded">
