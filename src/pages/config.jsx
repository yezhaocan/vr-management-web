            ...
            <CardContent>
              <div className="text-muted-foreground space-y-2 text-sm">
                <p className="flex items-start">
                  ...
                  配置数据以JSON格式存储，支持嵌套对象和数组
                </p>
                <p className="flex items-start">
                  <span className="mr-2 text-primary">•</span>
                  系统会自动获取第一条配置记录，如果没有记录则创建新记录
                </p>
                <p className="flex items-start">
                  <span className="mr-2 text-primary">•</span>
                  每次保存都会更新同一条记录，确保配置的唯一性
                </p>
                <p className="flex items-start">
                  <span className="mr-2 text-destructive">•</span>
                  JSON格式错误时无法保存，请确保格式正确
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  {jsonValid ? (
                    <div className="flex items-center text-green-500 text-sm font-normal bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-md border border-green-200 dark:border-green-800">
                      <CheckCircle className="h-4 w-4 mr-1" />
                      JSON格式正确
                    </div>
                  ) : (
                    <div className="flex items-center text-destructive text-sm font-normal bg-destructive/10 px-2 py-1 rounded-md border border-destructive/20">
  ...
  "settings": {
    ...
    "notifications": true
                  ...
                  <div className="text-xs text-muted-foreground mt-2 flex items-center justify-end">
                    ...
                    {existingConfig
                      ? `最后更新: ${existingConfig.updatedAt ? new Date(existingConfig.updatedAt).toLocaleString() : '未知'}`
                      : '暂无配置数据'}
                  </div>
                </div>
