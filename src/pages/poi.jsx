...
export default function POIManagement(props) {
  ...
  // 搜索POI
  const filteredPoiList = poiList.filter(
    (poi) =>
      poi.name?.toLowerCase().includes(searchKeyword.toLowerCase()) ||
      poi.description?.toLowerCase().includes(searchKeyword.toLowerCase())
  );
    ...
    <AuthGuard $w={$w}>
      <div style={style} className="w-full h-full space-y-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col justify-center items-center h-96 border rounded-lg bg-card text-card-foreground">
            ...
            <Button variant="outline" onClick={handleAddPoi}>
              创建第一个POI
            </Button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredPoiList.map((poi) => (
              <Card
                key={poi._id}
                className="bg-card text-card-foreground border border-border shadow-sm hover:shadow-md hover:border-primary transition-all duration-200 group"
              >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-muted/30 rounded p-2 border border-border">
                      ...
                      <div className="font-mono font-medium">
                        {poi.latitude?.toFixed(6) || '0.000000'}
                      </div>
                    </div>
                    <div className="bg-muted/30 rounded p-2 border border-border">
                      <div className="text-muted-foreground mb-1">经度</div>
                      <div className="font-mono font-medium">
                        {poi.longitude?.toFixed(6) || '0.000000'}
                      </div>
                    </div>
                  </div>
