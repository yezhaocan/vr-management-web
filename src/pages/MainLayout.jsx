...
const menuItems = [
  ...
  },
  {
    ...
    label: '景区管理',
    icon: <Image size={20} />,
    path: '/workspace/scenic-management',
  },
  {
    id: 'drone',
    label: '无人机管理',
...
// 辅助组件：菜单项
function NavItem({ icon, label, isOpen, active = false, onClick }) {
  return (
    <Button
    ...
    >
      ...
      {isOpen && active && (
        <div className="ml-auto w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(24,144,255,0.6)]" />
      )}
    </Button>
  );
}
...
// MainLayout 组件
export function MainLayout({ children, $w }) {
    ...
    <ThemeProvider defaultTheme="dark" storageKey="vite-ui-theme">
      <div className="min-h-screen bg-background flex flex-col transition-colors duration-300">
        ...
        <div className="w-full fixed top-0 left-0 right-0 z-50">
          <Header $w={$w} title={currentTitle} subtitle="智慧景区无人机管理系统" />
        </div>
        ...
        <div className="flex flex-1 overflow-hidden w-full pt-16">
          ...
          >
            <div className="main-content-area p-6">{children}</div>
          </main>
        </div>
      </div>
    </ThemeProvider>
