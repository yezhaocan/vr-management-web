...
export default function FlightTaskPage(props) {
  ...
  const loadMissions = async () => {
    try {
      ...
      const missionsWithDroneInfo = await Promise.all(
        (result.records || []).map(async (mission) => {
          if (mission.droneId) {
            try {
  ...
  const getFilteredMissionsByTab = () => {
    const filteredBySearch = missions.filter((mission) => {
      const matchesSearch =
        ...
        mission.droneModelSn?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const getStatusBadge = (status) => {
    const variants = {
      pending:
        'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100/80',
      executing:
        'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300 hover:bg-orange-100/80',
      completed:
        'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 hover:bg-green-100/80',
    };
    const labels = {
      pending: '待执行',
      executing: '执行中',
      ...
      <div style={style} className="space-y-6 animate-in fade-in duration-500">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-muted p-1 rounded-lg">
            <TabsTrigger
              value="pending"
              className="flex items-center space-x-2 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm"
            >
              ...
              {statusCounts.pending > 0 && (
                <Badge
                  variant="secondary"
                  className="ml-1 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"
                >
            ...
            >
              ...
              {statusCounts.executing > 0 && (
                <Badge
                  variant="secondary"
                  className="ml-1 bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300"
                >
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredMissions.map((mission) => (
                  <Card
                    key={mission._id}
                    className="bg-card border-border shadow-sm hover:shadow-md hover:border-primary/50 transition-all duration-200"
                  >
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-medium text-foreground">
                            {mission.droneModelSn || mission.droneId || '未选择'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-muted-foreground">航线</span>
                          <span className="font-medium text-foreground">
                            {mission.airlineName || mission.airlineId || '未选择'}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                      ...
                      <div className="flex justify-between items-center pt-3 border-t border-border mt-2">
                        <div className="flex space-x-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleEditTask(mission)}
                        ...
                        <div>
                          <Button
                            ...
                            disabled={
                              !canExecuteMission(mission) || executingMissions.has(mission._id)
                            }
        ...
        <Dialog open={showForm} onOpenChange={setShowForm}>
          <DialogContent className="w-[800px] h-[800px] bg-background border-border text-foreground transition-all duration-300 flex flex-col p-0">
            <DialogHeader className="p-6 pb-2 border-b border-border">
              <DialogTitle>{editingMission ? '编辑飞行任务' : '新建飞行任务'}</DialogTitle>
            </DialogHeader>
